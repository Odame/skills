---
name: unban-term
description: Stop plainspeak flagging a word or phrase, removing it from the banned-term list and adding it to the wordfreq and idiom allowlists. Trigger on any request to unban or allowlist a term for this checker, such as "stop flagging X", "unban 'leverage'", "'utilize' is fine, stop blocking it".
---
...
```bash
uv run --project "${CLAUDE_PLUGIN_ROOT}" plainspeak unban <term>
```
